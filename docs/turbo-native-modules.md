...
1. The file must be named Native<MODULE_NAME>
...
Here, the specs package name `(com/nativelocalstorage)` is determined by the `codegenConfig.android.javaPackageName` and generated interface file name and interface name is determined by the Spec's file name suffixed with `Spec`.
:::
...
Here, the names of the generated files/folders are determined by `codegenConfig.name` and the names of the protocol, base class, and JSI binding are determined by the Spec's file name 
:::
